...
print('总单词个数为：', len(wordlist))
uniqWord = list(set(wordlist))
print('不重复单词个数为：', len(uniqWord))

print('不重复单词列表：')
for i in range(0, len(uniqWord), 5):
    for j in uniqWord[i:i+5]:
        print('{:<20}'.format(j), end='')
    print()

# 词频统计打印
wordDct = {}.fromkeys(uniqWord, 0)
for i in wordlist:
    wordDct[i] += 1
# w = sorted(wordDct.items(), key=lambda x: x[1], reverse=True)
ww = sorted(zip(wordDct.values(), wordDct.keys()), reverse=True)
print('\n\n词频统计结果：')
